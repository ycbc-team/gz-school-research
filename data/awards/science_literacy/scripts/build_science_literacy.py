...
def main():
    source = next(RAW.glob("附件1 *科学素养大赛*获奖*名单.xlsx"), None)
    if source is None:
        raise FileNotFoundError("缺少科学素养大赛学生获奖名单 xlsx")
    rows = workbook_rows(source)
    header_index = next(i for i, row in enumerate(rows) if "学校/单位名称" in row)
    headers = {name: i for i, name in enumerate(rows[header_index]) if name}
    ...
    for row in rows[header_index + 1:]:
        ...
        record = {"category": value(row, "参赛项目"), "district": value(row, "属地"),
                  "school": school, "group": group, "student": value(row, "学生姓名"),
                  "coach": value(row, "指导老师", "指导老师"), "award": value(row, "奖项")}
        ...
        hits = matcher.resolve(school, preferred_adcode=adcode, preferred_stage=group[:2], strategy="all")
    ...
    parsed = {"metric": "science_literacy_awards", "competition": "广州市中小学生科学素养大赛",
              "year": 2025, "source_file": source.name, "total_records": len(records),
              "skipped": skipped, "records": records, "compiled_schools": len(compiled)}
    PARSED.mkdir(exist_ok=True)
    (PARSED / "science_literacy_2025.json").write_text(json.dumps(parsed, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    DIST.mkdir(exist_ok=True)
    (DIST / "compiled.json").write_text(json.dumps({sid: {"science_literacy_awards": data} for sid, data in compiled.items()}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"[2025] records={len(records)} matched={sum(bool(r['school_ids']) for r in records)} skipped={len(skipped)} schools={len(compiled)}")
