...
DROP_CAMPUS = [
    ...
    # 2026-09-24：广州市西关外国语学校中学部 gz-440103-1df8fc85 与初中部 gz-440103-b41a3512
    # 同址冗余（坐标 ~110m，均为西外本部办学点；官方附件1 仅列校本部/彩虹桥/文昌南，无「中学部」名）。
    # 剔除后旧名承继初中部（REMOVED_POI_ALIAS），搜索「中学部」仍命中初中部实体、详情页有招生。
    '广州市西关外国语学校中学部',
]
...
OFFICIAL_MIDDLE_ALIAS = {
    ...
    '广州市天河区暨南第一实验学校': '暨南大学附属中学',  # 2026-09-23 天河附件6 17号官方新名；实体为暨大附中（历史反推锚定）
    ...
    '广州市第89中学': '广州市第八十九中学',  # 2026-09-23 下沉：官方阿拉伯数字简称（天河附件6），数字→中文无法由 core 名桥接（业务 SCHOOL_NORM 同名条目已删）
    # 海珠区
...
REMOVED_POI_ALIAS = {
    ...
    'primary|中山大学附属小学': '中山小学',  # 旧名→更名承继（2026-09-23 用户确认：2021 更名广州市海珠区中山小学）
    # 2026-09-22 用户确认：刘村社区旧村改造项目（荷村复建工程项目）AG0222001地块九年制学校（暂定名）
    ...
    # 2026-09-24：西外中学部（1df8fc85）与初中部（b41a3512）同址冗余 → 承继初中部
    'middle|广州市西关外国语学校中学部': '广州市西关外国语学校(初中部)',
    ...
    # —— 2026-09-23 番禺/天河 POI 未匹配处置补承继（并入/更名同一实体，旧名挂别名）——
    'primary|低涌小学': '海傍学校',                      # 官方预算信息确认并入海傍学校；2026 官方地段 zone 含「海傍村、低涌村户籍」
    'primary|石楼镇沙南小学': '海鸥学校',                 # 2010 年并入海鸥学校，现为海鸥学校沙南校区
    'primary|茭塘小学': '广东仲元中学莲花湾学校',          # 2020 并入山海连城学校 → 2025-05 搬迁更名广东仲元中学莲花湾学校（官方公告 post_10250808）
    'primary|明德实验小学': '明德广地实验学校',            # 同址重复（广地花园内九年一贯民办），POI 名「明德实验小学」为俗称
    'primary|广东番禺中学附属学校-番中附小': '广东番禺中学附属学校',  # 九年一贯学校小学部（官方 2026 预告「番禺中学附属学校（小学部）」），同址重复
    # —— 2026-09-23 天河 POI_NAME_FIX 更名归一后旧名承继 ——
    'primary|暨南大学附属小学': '暨南大学附属实验学校',      # 官方名「暨南大学附属实验学校（小学部）」（附件7 企事业办）
    'primary|暨南大学华文学院-长征小学': '广州市天河区长征小学',  # 官方名「广州市天河区长征小学」（附件7 #6 企事业办，位于暨大华文学院内）
    'primary|广州市育华教育集团长虹学校': '广州市天河区长虹小学',  # 官方名「长虹小学」（附件8 #28 民办）
}
...
OFFICIAL_HIGH_ALIAS = {
    ...
    '广州市广外实验中学': '广东外语外贸大学实验中学',  # 2026 区文件别称（招考办汇总表用「广东外语外贸大学实验中学」）
    '广东外语外贸大学实验中学（北校区）': '广东外语外贸大学实验中学(北校区)',
    '广州大同中学': '广州大同中学(高中部)',
...
OFFICIAL_PRIMARY_ALIAS = {
    ...
    "前进小学": "前进小学",
    # 2026-09-23 海珠中路小学：法人裸名挂两校区实体（七株榕/海珠中，多校区并列招生，
    #   resolve_all 裸法人名 → 两校区；此前 matchNorm 把「海珠」当区名剥离致两侧不对称失配）
    "海珠中路小学": ["海珠中路小学(七株榕校区)", "海珠中路小学(海珠中校区)"],
    # 海珠（中山小学 = 中山大学附属小学：海珠区教育局 2021-11-01 批复同意
    #   「中山大学附属小学」办学名称变更为「广州市海珠区中山小学」；海珠公办初中派位组表
    #   官方原文用「中山小学」（faq_09 第五组）——2026-09-23 用户确认）
    "车陂小学": "车陂小学",
    "东圃小学": "东圃小学",
    "羊城花园小学": "羊城花园小学",
    "盈彩美居小学": "盈彩美居小学",
    ...
    "长征小学": "广州市天河区长征小学",  # 2026-09-22 POI_NAME_FIX 后实体名
    ...
    "暨南大学附属实验学校（小学部）": "暨南大学附属实验学校",  # 2026-09-22 POI_NAME_FIX 后实体名
...
POI_NAME_FIX = {'广州市第十三中学文德校区': '广州市第十三中学(文德校区)',
                ...
                '金盆学校': '良田第三小学(金盆校区)',
                # 2026-09-23 用户定：石碁中心小学大龙校区归位石碁镇中心小学（官方 2026 地段表 zone
                # 明确含「大龙校区：大龙村户籍…金龙居委…」），规范为括号式校区名使 resolve_all 吸附
                '石碁中心小学大龙校区': '石碁镇中心小学(大龙校区)',
                ...
                # 2026-09-23 用户确认：中山大学附属小学 2021-11 海珠区教育局批复更名
                #   「广州市海珠区中山小学」（公办派位组表官方原文用「中山小学」）——
                #   新名做主名（实体 name），POI 原名仅作地图标签保留（school_id 不变），旧名挂别名承继
                '中山大学附属小学': '中山小学',
                ...
                '芦荻西小学(桃源校区)': '广州市第四中学附属芦荻西小学(桃源校区)',
                # —— 2026-09-22 天河 POI 未匹配处置：高德名/旧名归官方名（附件7 企事业办计划在册）——
                '暨南大学华文学院-长征小学': '广州市天河区长征小学',  # 高德名带学院前缀；官方=广东省农垦总局与暨大华文学院合办子弟学校（粤垦路88号）
                '暨南大学附属小学': '暨南大学附属实验学校',  # 旧名；2026 附件7 #3 官方名=暨南大学附属实验学校（小学5班）
                '广州市育华教育集团长虹学校': '广州市天河区长虹小学',  # 育华集团旗下民办完全小学；2026 附件8 #28 长虹小学 3班128人
}
...
# 2026-09-23 增补：官方阿拉伯数字简称（天河附件6「广州市第N中学」）→ 中文全称法人多校区。
# 数字→中文无法由 core 名桥接（实体核心为中文全称），须显式挂全部同法人 middle 校区，
# 才能保持 resolve_all 法人展开（school_ids 多校区）与 SCHOOL_NORM 归一前行为一致。
SHARED_LEGAL_ALIAS = {
    '广铁一中铁英学校': ('广州市番禺区广铁一中铁英学校(东校区)', '广州市番禺区广铁一中铁英学校(西校区)'),
    '广州市第18中学': ('广州市第十八中学', '广州市第十八中学车陂校区'),
    '广州市第113中学': ('广州市第一一三中学(乐学校区)', '广州市第一一三中学(东方校区)'),
    '广州市第75中学': ('广州市第七十五中学(燕塘西校区)', '广州市第七十五中学(天平架校区)'),
}
...
# 石碁中心小学大龙校区（POI 原名，实体经 POI_NAME_FIX 归为「石碁镇中心小学(大龙校区)」）：
# POI 原名挂回别名，官方「石碁镇中心小学」记录 resolve_all 展开吸附大龙校区（2026-09-23 用户定）
if attachAlias('primary', '石碁镇中心小学(大龙校区)', '石碁中心小学大龙校区'):
    aliasHit += 1
else:
    print('  [POI原名别名未命中] 石碁中心小学大龙校区 -> 石碁镇中心小学(大龙校区)')
...
EXTRA_ENTITY_ALIAS = {
    ...
    'gz-440112-d3055548': ['铁铮学校'],    # 广铁一中铁铮学校(东校区)：法人名（官方附件5 对口直升按“铁铮学校”整体公布，东/西校区共享该记录，走通用多校区规则）
    'gz-440112-7127244c': ['铁铮学校'],    # 广铁一中铁铮学校(西校区)：同上（POI 同坐标于铁铮学校主实体）
    'gz-440112-15d63ae7': ['黄埔区CPPQ-A4-2地块（长岭、雅居建设项目）九年制学校', '黄埔区CPPQ-A4-2地块（长岭.雅居建设项目）九年制学校', '黄埔区CPPQ-A4-2地块（长岭·雅居建设项目）九年制学校'],  # 玉岩附属开发区实验学校：官方暂定名（附件4 小学用顿号转录/附件7 初中官方原文用点，两形态都挂；2026-09-23 补官方原文全角中点·变体——业务 SCHOOL_NORM 同名归一已删，由实体别名直接承接）
    # 2026-09-24 初中招生缺口修复（用户清单 24 所抽查结论第 2/4 类）：
    'gz-440113-c07e3ddb': ['毓贤学校'],      # 祈福毓贤学校初中部(毓正小学校区)：与主实体 7cdbe028（广州番禺区毓贤学校）同法人
                                              # （官方小学/初中表均名「毓贤学校」，祈福新邨）——resolve_all 双实体命中
    'gz-440113-7cdbe028': ['广州市番禺区毓贤学校'],  # 主实体持官方配额表全称（quota_matrix 原文）：norm 全等命中，避免
                                              # 双实体持「毓贤学校」后 backfill mnorm 兜底多候选宁缺（2026-09-24）
    'gz-440113-6a13dc63': ['二师番禺附中', '番禺二师附中'],  # 广东第二师范学院番禺附属初级中学：用户检索用简称（官方名已能命中）
    'gz-440112-12b0184b': ['玉泉学校中学部'],  # 玉泉学校：官方附件7 直升组名「玉泉学校」，用户检索「中学部」形态补别名
}
...
PRIMARY_CAMPUS_ALIAS = {
    ...
    # 海珠（2026-09-23：执信琶洲实验学校（小学部）= 广州市执信中学琶洲实验学校 primary 实体，官方转录名挂别名）
    "执信中学琶洲实验学校（小学部）": "广州市执信中学琶洲实验学校",
    # 越秀（2026-09-23：华侨外国语学校（小学部）= 「广州市华侨外国语学校-华侨小学」primary 实体）
    "广州市华侨外国语学校（小学部）": "广州市华侨外国语学校-华侨小学",
    # 番禺
    "化龙镇复甦小学": "复苏小学",
    "石碁镇茂生小学": "茂生纪念学校",
    "石碁镇韵琴小学": "南浦韵琴小学",
    ...
    "毓贤学校": "广州番禺区毓贤学校",
    "湖南师范大学附属黄埔实验学校": "湖南师范大学附属黄埔实验学校北校区",
    # 荔湾（华侨小学校本部 = 南岸路校区（西校区）：荔湾招生地段原文「华侨小学（校本部）」
    #   南源街荔港南湾/电业社区，官方地址南岸路61号 = POI 荔湾区华侨小学(西校区)——2026-09-23 用户确认）
    "华侨小学（校本部）": "荔湾区华侨小学(西校区)",
    # 荔湾（芳村小学沙涌校区：2023 区小学一览表/发改委开办改造项目官方来源，地址沙涌新村中街2号，
    #   2026-09-23 高德核实补 POI gz-440103-9ff7c5c3；实体名半角括号与现有一致）
    "芳村小学（沙涌校区）": "芳村小学(沙涌校区)",
    # 海珠（更名正式全名：中山大学附属小学 → 广州市海珠区中山小学，2021-11 区教育局批复）
    "广州市海珠区中山小学": "中山小学",
}
...
for _e in entities:
    for _k in ([matchNorm(_e['name'])] + [matchNorm(_a) for _a in (_e.get('aliases') or [])]):
        if _k:
