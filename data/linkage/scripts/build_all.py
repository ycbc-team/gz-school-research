...
管线（A/B/C 分层）：
  ...
  C 加工（canonical → dist 运行时产物）：
      backfill_school_ids.py     SchoolMatcher：校名 → school_id / middle_school_ids
      optimize_redundancy.mjs    去冗余（sz 稀疏化、删 admitted:false 行、msi 裁剪）
      build_ranking_middle.py    初中升学信号聚合（读 dist 表）
  校验：
      check_special_matrix_snapshot.py  special_matrix 业务快照
      check_dist_snapshots.py           其余四产物业务快照
