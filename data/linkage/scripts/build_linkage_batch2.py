# -*- coding: utf-8 -*-
"""从全量第二批次数据过滤省市属 11 所高中，生成规范表（B 层）
说明：分数表为"初中-高中对"聚合行；空分数=有指标但未完成录取（未达控制线/无人报考）。
指标计划数 n_ji 需与名额分配计划明细表合并（quota 表）。

输出：data/linkage/parsed/canonical/batch2_scores.json（清洗后规范表，无 middle_school_ids——
     由 C 层 backfill_school_ids.py 生成）。
"""
import json, re, os
...
ALL = os.path.join(LINK, 'parsed', 'batch2_scores_all.json')
OUT = os.path.join(LINK, 'parsed', 'canonical', 'batch2_scores.json')
