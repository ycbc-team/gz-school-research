...
export function createBadgesApi(loaders: DataLoaders) {
  ...
  /**
   * 学校徽章组：区域 · 学段 · 民办 · 省市属/区属。
   * 省市属/区属徽章对应指标到校（名额分配）批次：省市属示范高中名额面向全市、
   * 区属示范高中面向本区（官方口径：省市属/区属公办优质普通高中），只在高中 tab 显示。
   */
  function schoolBadges(
    ...
    // 省市属/区属徽章对应指标到校批次（官方：省市属名额面向全市、区属面向本区），只在高中 tab 显示
    if (stage === 'high' && opts.rec) {
      if (opts.rec.category === '省市属示范') out.push({ text: '省市属', cls: 'b-hcity' });
      else if (opts.rec.category === '区属示范') out.push({ text: '区属', cls: 'b-hdist' });
    }
    return out;
  }
