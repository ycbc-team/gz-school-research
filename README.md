...
路由 `#/`（入口）/ `#/map`（地图）/ `#/support`（口碑信号），数据经 `src/data/` 从 `data/*.json`（真源）加载，
...
- 七类配色：小学·普通（浅灰蓝）/ 小学·口碑（蓝）/ 初中·普通（暖浅灰）/ 初中·口碑（红）/ 高中普通（灰蓝）/ 高中区属示范（绿）/ 高中省市属示范（琥珀），口碑校按客观信号评分（≥2分发口碑Badge）
...
- 6 项指标可选：自招人数（绝对值）/ 自招比例（÷名额分配符合资格考生数）/ 区属指标数 / 区属指标比例 / 省市属指标比例 / 指标×高中特控率（Σ(区属指标数×目标高中特控率)÷Σ(有特控率数据的高中指标数)）
...
旧版"网传口碑学校"以自媒体梯队榜为入口、人工判定"有支撑/部分支撑"，存在一梯队泛化、收录无门槛、判定不可复算等问题。2026-09-14 重构为**信号驱动的可计算口径**：

**四维度信号**（真源：`data/{primary,middle}/tier1_schools_all.json`）：
1. **客观支撑**：历史省/市一级·重点称号（1994-2005 评定、2005 停评，仅作历史客观记录）、学位预警、招生班数
2. **一流教育集团**：8 大品牌（华附/省实/执信/二中/广附/广雅/铁一/清华附中湾区）+ 省市属集团 + 核心校为省一级的区属集团；区分核心校/成员
3. **初中专属**：网传中考喜报（注明年份与口径，民间非官方）、官方录取分数线（2025/2026，810 分制）、自主招生录取人数（2026 自招资格名单统计）
4. **小学专属**：是否直升到口碑初中（`xiaoshengchu_2026.json` 直升关系）

**评分规则**：初中 v3（2026-09-14 第三轮）：移除高中录取线计分（与初中教学质量无因果、对纯初中不公平，字段保留作展示）；自招提权为核心信号（≥80+1、≥50+0.5）；省一级+0.5封顶、一流集团核心+1.5、喜报时效加权（2024++1/2020-23+0.5/≤2019不计），总分≥3.0 为「口碑」（v1→v2→v3：52→24→19）；小学沿用 v1 权重，≥2.0 为「口碑」。每条判定可追溯至信号来源 URL，查不到标 `data_gaps`。

**UI 语义**：口碑 Badge 仅授予 `reputation.level==='口碑'`；不支撑学校显示「挂牌」灰标；待观察学校在口碑信号页展示但不发 Badge。

**当前覆盖**：小学 58 所（口碑 17 / 待观察 34 / 不支撑 7，沙步小学已停办移除）；初中 54 所（口碑 52 / 待观察 2）。
...
- [x] 网传"口碑学校"核验（58 所，新口径：口碑 17 / 待观察 34 / 不支撑 7；四维度客观信号：历史称号·一流集团·直升口碑初中·学位预警）
- [ ] 白云、黄埔招生数据（低优先级）
- [ ] 未匹配 42 所逐校核实（新建校/更名）
...
- [x] 各区网传口碑学校初中名单 + 客观数据核验（54 所，v3口径：口碑 19 / 待观察 30 / 不支撑 5；移除高中录取线计分、自招提权为核心信号；网传7硬共识7/7，详见 docs/2026网传第一梯队初中搜索与对比报告.md）
- [x] 中小学合并地图（四类配色 + 筛选）
...
- [x] **P3** 区属非示范集团 + 小学集团全量（7区85个教育集团/334所成员校，`data/registry/education_groups.json`，覆盖清单见 `docs/education-groups-coverage/集团成员覆盖清单_P3.md`；POI精确命中157/变体命中167/7区内缺失2/远郊8；缺失2所=三元里中学（在办未收录）+广龙地块配建学校（建设中））
