...
- 共 985 所：荔湾 86 / 越秀 82 / 海珠 127 / 天河 133 / 白云 246 / 黄埔 110 / 番禺 201
- 已合并同校重复 POI；番禺含 backfill 补充点；废弃/错误点位逐校核实后清除（留痕见「荔湾 POI 未匹配处置经验」）
- 通过翻页采集突破单区 100 条上限
...
- 当前规模：7 区 755 条 records（荔湾 75 / 越秀 69 / 海珠 99 / 天河 93 / 白云 164 / 黄埔 102 / 番禺 153）；各区 `unmatched`（官方名单未绑定 POI）已全部清零
- `poi_leftover`（POI 池剩余未匹配点位）：荔湾已清零；其余区逐校核实进行中（流程见「荔湾 POI 未匹配处置经验」）
- 来源：各区教育局 2026 官方招生文件（官网 PDF/图文，链接见 build 脚本 SOURCE_URLS）；海珠为官网图片 OCR、天河为扫描 PDF OCR，个别字可能有误差，均以官方原文件为准
- 快照守护：`data/primary/enrollment/test/check_enrollment_snapshot.py`（7 区 755 条基线），数据变更显式 `--update-snapshot`
...
- [x] 7 区小学招生统一构建（2026，755 条绑定，ABC 三层：转录/匹配/合并）
- [x] 网传"口碑学校"清单保留为学校名单（58 所，保留历史称号/集团/学位预警/招生班数等源数据，2026-09-15 口碑判定已移除，不再做梯队评分）
- [ ] 未匹配 POI 逐校核实（荔湾 9 点已闭环 2026-09-22，流程见「荔湾 POI 未匹配处置经验」；其余区进行中）
