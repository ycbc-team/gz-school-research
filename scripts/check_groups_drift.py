...
重跑生产脚本 merge_groups.py 到临时文件，与入库产物 data/registry/education_groups.json 对比：
- 完全一致 → 通过（产物=脚本输出，说明源改动已同步）
- 有差异 → 失败并列出 丢失/错配/新增 三类差异（提醒"改了源忘了重跑产物"）

用法：python3 scripts/check_groups_drift.py
排除：本脚本只跑本地数据处理（merge_groups/match_poi 均为本地文件读取，无高德等联网 API）。
"""
...
def main():
    ...
    r = subprocess.run(["python3", os.path.join(ROOT, "scripts/merge_groups.py"), tmp],
