...
def main():
    ...
    for pno in sorted(g6, key=int):
        ...
        for r in range(1, nrows):
            ...
            # 待重读列：col1 空则重读；21 列中空的格重读
            todo = []
            if sheng is None:
                todo.append(1)
            for i in range(SZ):
                ci = i + 3
                if clean(pg['data'].get(str(ci), {}).get(str(r), '')) is None:
                    todo.append(ci)
            # Σ>col1 说明 col1 漏 → 重读 col1
            if sheng is not None and szsum > sheng:
                todo.append(1)
            for ci in sorted(set(todo)):
                if ci + 1 >= len(cols):
                    continue
                x0 = cols[ci] + 4
                x1 = cols[ci+1] - 4
                ...
                txt = ocr_crop(crop)
                if txt:
                    pg['data'].setdefault(str(ci), {})[str(r)] = txt
                need += 1
                tot_cells += 1
