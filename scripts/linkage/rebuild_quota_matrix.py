...
# school_id 修正：quota 校名 -> 注册表实体 school_id（修复此前 md5 生成/误配的假 id，
# 使 minban 民办判定与详情页 stage 映射回归正确）。均经 entities 名/别名核对。
SID_FIX = {
    '广州市海珠区华立学校': 'gz-440105-8371ce9c',          # 华立学校 middle 民办
    '广州市荔湾区海龙博雅中英文学校': 'gz-440103-957da59e',  # 海龙博雅 primary 民办
    '广州荔湾爱莎文华学校': 'gz-440103-5143f5a1',          # 爱莎文华 high 民办
    '广州市天河区同仁实验学校': 'gz-440106-77787b4a',      # 同仁实验学校小学部 middle/primary 民办
    '广州市天河区东风学校': 'gz-440106-24ff78f9',          # 东风学校 middle 民办
    '广州市黄埔区铁英中学': 'gz-440112-c80ac6ac',          # 广铁一中铁英中学 初中（别名黄埔铁英）
    '广州市番禺区丽江学校': 'gz-440113-2264148c',          # 丽江小学（九年制同校实体）
    '广州市番禺区祈福新邨学校': 'gz-440113-8ef59a4c',      # 初中部实体（祈福新邨，POI 名同 id；此前误用小学实体 1963cc5e）
}
...
def main():
    ...
    for pno in sorted(PAGE_DIST):
        ...
        for item in plan:
            ...
            if rec['school'] in SID_FIX:
                rec['school_id'] = SID_FIX[rec['school']]
            schools.append(rec)
