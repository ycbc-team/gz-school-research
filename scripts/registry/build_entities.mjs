...
const OFFICIAL_MIDDLE_ALIAS = {
  ...
  '六中逸景校区': '海珠区六中珠江中学',
  '赤岗中学': '广州市景中实验中学',
  '人和二中': '白云六中实验中学(空港校区)',            // 2022-05 人和二中+人和三中合并更名白云区六中实验中学（空港校区），南方+官方核实                      // 2013-04 赤岗中学正式更名景中实验中学（人民网/天眼查核实）；与名单"景中"同实体
  // 白云区
...
for (const [sid, als] of Object.entries(EXTRA_ENTITY_ALIAS)) {
  ...
  if (e) {
    for (const a of als) {
      for (const v of withDistrictVariants(normName(a))) if (v !== normName(e.name)) e.aliases.add(v);
    }
  } else {
    console.log('  [EXTRA别名未命中实体]', sid);
  }
}
