...
1. 小程序地图信息卡交互形态：`cover-view` 底部抽屉（移动端习惯）还是右上角浮卡（对齐 Web）？—— 建议底部抽屉，移动端拇指可达。

2. 升学通道 / 政策页是否进小程序首批（P3），还是先只做地图 + 详情（用户使用频率最高）？—— 建议首批只做 P0–P2，P3 按数据反馈再定。

3. 分包命名与 tabBar 形态：当前 index/map/support 为普通页面（无 tabBar），是否要改 tabBar 布局？